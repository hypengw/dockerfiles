...
then
  ...
  /usr/sbin/eips 11 33 "Listening on: $myip:8384"
fi
}

#check if sshd is running
if pgrep syncthing &> /dev/null; then
  /usr/sbin/eips 11 32 "Syncthing is RUNNING"
  show_ip
else
  /usr/sbin/eips 11 32 "Syncthing is NOT running"
fi

logfile=/mnt/us/extensions/syncthing/syncthing.log
eips 11 35 "log:"
col=35
if [ -f $logfile ];then
